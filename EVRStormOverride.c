...
modded class EVRStorm
{
	...
	*/
	...
	// -----------------------------------------------------------
	// ФИКС: "конец шторма" висел на EndPhaseServer() - оказалось, это
	// НЕ конец, а третья игровая фаза шторма (после Init/Mid), после
	// которой шторм ещё идёт какое-то время (подтверждено логом сервера:
	// "EndPhaseServer" -> "Phase Length 220" -> ... -> "Phase Length 0"
	// -> и только ТУТ настоящий конец). Из-за этого "шторм закончился"
	// писалось на пару минут раньше реального конца, а следующий тик
	// UpdateServer() тут же видел сброшенный нами флаг и писал "начался"
	// заново - хотя шторм и не прерывался. Настоящий конец - деструктор
	// ~EVRStorm() (см. override ниже), это подтверждено его же
	// собственным принтом в логе сервера ("[Namalsk][EVRStorm]: ~EVRStorm"),
	// прямо перед которым идёт "Could not start event EVRStorm as it is
	// on cooldown" - то есть событие реально закончилось и ушло в кулдаун.
	// -----------------------------------------------------------

	// -----------------------------------------------------------
	// НОВОЕ: настоящий конец шторма - уведомление в Discord + уборка
	// нашего состояния, чтобы следующий шторм стартовал чисто.
	//
	// ФИКС: "Function '~EVRStorm' is marked as override, but there is no
	// function with this name in the base class" - в этом движке
	// деструкторы не считаются обычными переопределяемыми методами (в
	// отличие от UpdateServer/GetEventPosition и т.д. выше), "override"
	// им не положен, даже если деструктор в базовом классе точно есть.
	// Убрали "override" - деструктор modded class просто объявляется
	// как есть и цепляется автоматически при уничтожении объекта (как и
	// конструкторы в этом движке - см. пример TraderPlusIDsSettings,
	// там тоже без super()).
	// -----------------------------------------------------------
	void ~EVRStorm()
	{
