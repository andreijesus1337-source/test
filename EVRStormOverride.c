...
modded class EVRStorm
{
	...
	*/
	...
	//
	// ВАЖНО: единственное здесь, что не подтверждено напрямую исходником -
	// сам факт, что деструктор можно переопределить через "override" (по
	// аналогии с обычными методами вроде UpdateServer выше). Имя ~EVRStorm
	// подтверждено логом на 100%. Если компилятор выдаст ошибку именно на
	// этой сигнатуре - пришлите текст, поправим синтаксис деструктора.
	// -----------------------------------------------------------
	override void ~EVRStorm()
	{
